...
router.post('/register', async (reg, res) => {
    ...
    const hashPassword = await bcrypt.hast(reg.body.password, salt)
    ...
    try {
        ...
        res.status(2000).json(result(1, 'Register User Success!', saveUser))
    } catch (error) {
...
router.post('/login', async (req, res) => {
    ...
    //check password
    const validPwd = await bcrypt.compare(req.body.password, user.paaword )
